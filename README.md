...
| **Tensix コア内部(ブロック解説+FW深掘り)**: 5 RISC-V/Unpacker/FPU/SFPU/Packer/L1 + tt-metalソースから読み解いた2フェーズ起動・NCRISC IRAM・TT-NN→Kernelマッピング・DENNO対比 | [tensix-core/tensix_core_notes.md](tensix-core/tensix_core_notes.md) | [図解(起動アニメ+メモリマップ)](https://mstk13.github.io/MANABI/tensix-core/tensix_core.html) |
...
| **ニューラルネットの学習(勾配降下)**: 学習ループ(順伝播→損失→逆伝播→更新)+ 山下りを実際に動かせる。学習率を変えて発散も体感。なぜ学習は推論より重いか | [training/training.html(図)](https://mstk13.github.io/MANABI/training/training.html) | [図解(山下り操作)](https://mstk13.github.io/MANABI/training/training.html) |
